...
// State
// Busy mode — when ON, the bot replies with a short "I'm busy" message to
// everyone instead of generating a full AI response.
var busyMode = false;
// BUSY_PROMPT is an ADDITIVE layer appended to the normal AI_PROMPT when busy
// mode is on. The bot still answers with full context (reads the message,
// sees images, keeps Jose Miguel's personality) but ALSO weaves in naturally
// that he's a bit busy right now and will attend to them shortly.
...
try {
  if (fs.existsSync(BLOCKED_FILE)) {
    var raw = JSON.parse(fs.readFileSync(BLOCKED_FILE, 'utf8'));
...
function getState() {
  ...
  return { botStatus:botStatus, stats:stats, pending:pending, qr:currentQR, who:whoName, waitMs:WAIT_MS, waitMin:WAIT_MINUTES, model:OPENAI_MODEL, blockedCount: blockedChats.size, busyMode: busyMode };
}
...
async function handleMsg(message) {
  ...
  d.timerId = setTimeout(async function() {
    ...
    // Build the system prompt. When busy mode is ON, we APPEND the busy-context
    // layer to the normal prompt so the AI keeps full context + personality but
    // also weaves in that Jose Miguel is a bit busy right now.
    var activePrompt = AI_PROMPT;
    if (busyMode) {
      activePrompt = AI_PROMPT + '\n\n' + BUSY_PROMPT;
    }
    // Full history context in both modes — busy mode still answers properly.
    var msgsForAI = [{ role: 'system', content: activePrompt }].concat(history);

    console.log('[ai] Calling', OPENAI_MODEL, 'for', name, '|', busyMode ? 'BUSY MODE (full context + busy layer)' : 'normal', '| msgs:', msgsForAI.length - 1);
    var reply;
    try {
...
app.post('/api/busy', function(req, res) {
  var on = (req.body || {}).on;
  busyMode = !!on;
  console.log('[busy] Busy mode', busyMode ? 'ENABLED' : 'DISABLED');
  pushEvent('system', null, busyMode ? 'Modo ocupado ACTIVADO — respondiendo brevemente' : 'Modo ocupado desactivado — respuestas normales');
  io.emit('state', getState());
